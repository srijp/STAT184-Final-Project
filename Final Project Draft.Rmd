...
CovidTotalAndPop <- #The tables of state population (StatePop) and the coronavirus count per state (sumStates) are joined into one big table
  ...
  mutate(lowcount = count <= 2000) #If the COVID count in the state is less than or equal to 2000, then TRUE will be assigned to the lowcount classifier. Otherwise, FALSE will be assigned.
...
CovidTotalAndPop %>% #Creates a graph comparing the COVID count and the state population.
  ...
  facet_wrap(~lowcount, scales='free')
```
...
#The dates displayed are from March to now because some states don't have data before March and some have extensive data before March, so filtering it makes it easier to compare and understand
Group0 <-
  CovidCasesAndPop %>%
  group_by(rank) %>%
