...
CovidTotalAndPop %>%
  arrange(desc(count))
ggplot(CovidTotalAndPop,aes(x=State,y=covid_pop_percent))+geom_bar(stat='identity',position='stack')+ geom_smooth(method = 'lm')
ggplot(CovidTotalAndPop,aes(x=count,y=statepop))+geom_point(aes(x=count,y=statepop))+aes(size=statepop)
ggplot(CovidTotalAndPop,aes(x=count,y=statepop))+geom_point(aes(x=count,y=statepop))+geom_smooth(method = 'lm')+xlim(0,10000)+ylim(0,30000000)
```
```{r}
UScovid <-
  NewCovidPlot %>%
...
ggplot(CovidTotalAndPop,aes(x=State,y=count))+geom_point(aes(x=State,y=count))+aes(size=statepop)
```
```{r}
NYandNJ <-
  CovidCasesAndPop %>%
