...
NoCounty <- covid_per_state[ , !(names(covid_per_state) %in% drop)] #Drops countyFIPS and County Name because we do not have use for county data
...
FullDataCOVID <- #Joins the Rank table (StateRank) and the COVID count per state per date table (CovidwithDate) into one
  StateRank %>%
  select(stateFIPS,rank) %>%
  left_join(CovidwithDate, by = c("stateFIPS" = "stateFIPS"))
...
```{r}
totalCasesperState <- #Creates a table of the total number of cases per state
  FullDataCOVID %>%
  group_by(State, rank) %>%
  summarise(count = max(confirmedcases))
totalCasesperState
totalCasesperState %>% #Creates a bar graph to show the COVID count per state
...
```{r}
#This table gets the State and state populations using ZipGeography
...
```{r}
CovidTotalAndPopPerState <- #The tables of state population (StatePop) and the coronavirus count per state (totalCasesperState) are joined into one big table
  totalCasesperState %>%
...
CovidTotalAndPopPerState %>%
  arrange(covid_pop_percent)
CovidTotalAndPopPerState %>% #Creates a bar graph that shows the COVID count in each state
...
```{r}
#Creates a scatterplot that shows the COVID count per state and the percent of the population who tested positive for COVID-19.
CovidTotalAndPopPerState %>%
  ggplot(aes(x=count,y=covid_pop_percent,size=statepop))+ #The size of the glyph  represents the state population
...
```{r}
#Creates a graph comparing the COVID count and the state population.
CovidTotalAndPopPerState %>%
  #Removed New York and New Jersey because they are outliers
  filter(State != "NY") %>%
  filter(State !="NJ") %>%
  ggplot(aes(x=count,y=statepop))+
  geom_text(aes(label=State),check_overlap = TRUE)+ #Each state is represented as a glyph as the state's acronym.
...
```{r}
FullCovidAndStatePop <- #Joins the full COVID data table (FullDataCOVID) and the state populations table (StatePop)
  StatePop %>%
  left_join(FullDataCOVID %>% select(rank,Date,confirmedcases),
             by = c("rank" = "rank"))
FullCovidAndStatePop
...
```{r}
CovidCoordinates <- #The statecoordinates table and the count of COVID cases per state (totalCasesperState) are joined
  totalCasesperState %>%
...
#Used log because count in some states are much larger than others, so this keeps the general proportions of COVID counts when comparing states. Count itself displays circle markers in some states that are too large
```
